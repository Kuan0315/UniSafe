...
import { fetchReports, createReport, upvoteReport, addComment, addReply } from '../../services/reportService';
...
const startRecording = async (): Promise<any> => {
    try {
...
const stopRecording = async (recording: any): Promise<string | null> => {
    try {
...
const convertSpeechToText = async (audioUri: string): Promise<string> => {
    ...
    return new Promise((resolve) => {
        setTimeout(() => {
            resolve("This is a mock speech to text result");
        }, 1000);
    });
};
...
export default function ReportScreen() {
    ...
    const [allReports, setAllReports] = useState<Report[]>(mockRecentReports);
    const [filteredReports, setFilteredReports] = useState<Report[]>(mockRecentReports);
    const [selectedDangerType, setSelectedDangerType] = useState('all');
    const [sortBy, setSortBy] = useState('latest');
    const [showSortModal, setShowSortModal] = useState(false);
    const [showCommentModal, setShowCommentModal] = useState(false);
    const [currentReportId, setCurrentReportId] = useState<number | string | null>(null);
    ...
    const loadReportsFromBackend = async () => {
        try {
            ...
            if (reports && reports.length > 0) {
                const mappedReports: Report[] = reports.map((report: any, index: number) => ({
                    id: report._id || `backend-${index}-${Date.now()}`, // Use _id directly or create unique ID
                    type: report.type || 'other',
                    title: report.title || 'Untitled Report',
                    description: report.description || '',
                    location: report.location || '',
                    time: new Date(report.time || report.createdAt).toISOString(),
            ...
            setAllReports(mockRecentReports);
    ...
    const handleCreateReport = async () => {
        ...
        try {
            ...
            // Create frontend report object with unique ID
            const newReport: Report = {
                id: createdReport._id ? createdReport._id : `local-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
                type: selectedReportType,
                title: reportDescription.split('.')[0] || 'New Report',
                description: reportDescription,
                location: reportLocation,
    ...
    const handleComment = (reportId: number | string) => {  // Change this
    ...
    const submitReply = async (commentId: number) => {
        ...
        try {
            ...
            await addReply(currentReportId.toString(), commentId.toString(), {
                text: replyText.trim(),
                anonymous: isAnonymous
            });
    ...
    const handleLikeComment = (reportId: number | string, commentId: number) => {
        const updatedReports = filteredReports.map(report => {
            if (report.id === reportId) {  // This will work with string or number
                const updatedComments = report.comments.map(comment => {
                    if (comment.id === commentId) {
    ...
    const filterAndSortReports = (query: string, dangerType: string, sort: string) => {
    ...
    const handleUpvote = async (reportId: number | string ) => {
        try {
            // Send upvote to backend
            await upvoteReport(reportId.toString());
    ...
    const submitComment = async () => {
        ...
        try {
            ...
            await addComment(currentReportId.toString(), {
                text: newComment.trim(),
                anonymous: isAnonymous
            });

            // Update local state with better ID generation
            const updatedReports = filteredReports.map(report => {
                if (report.id === currentReportId) {
                    return {
                        ...report,
    ...
    // Add this function to debug duplicate keys
    const checkForDuplicateKeys = (reports: Report[]) => {
        const ids = reports.map(r => r.id);
        const duplicates = ids.filter((id, index) => ids.indexOf(id) !== index);
        if (duplicates.length > 0) {
            console.warn('Duplicate report IDs found:', duplicates);
        }

        // Check comment IDs
        reports.forEach(report => {
            const commentIds = report.comments.map(c => c.id);
            const commentDuplicates = commentIds.filter((id, index) => commentIds.indexOf(id) !== index);
            if (commentDuplicates.length > 0) {
                console.warn(`Duplicate comment IDs in report ${report.id}:`, commentDuplicates);
            }
        });
    };
    ...
    useEffect(() => {
        checkForDuplicateKeys(filteredReports);
